...
console.log(
  `odp zad 1 Kod asynchroniczny wykona się dopiery gdy dostanie odpowiedź ze źródła .Kod taki nie czeka na wykonanie reszty kodu ,a wykona się dopieor po spełnieniu obietnicy`
);
...
promiseZad2
  ...
  .catch((err) =>
    console.log("Error zad 2 i 3", err, "Ups coś poszło nie tak!")
  );
...
asynchro().catch((err) =>
  console.log(err, `Error zad 4`, "Ups coś poszło nie tak")
);
...
promiseZad5
  ...
  .catch((err) =>
    console.log("Error zad 5 v1", err, "Ups coś poszło nie tak!")
  );
// v2
const asynchroZad5 = async (par) => {
  ...
  console.log(
    "odp zad 5 v2",
    `Wybrana postać nr ${par} to: ${postac.name}`,
    postac
  );
};
asynchroZad5(1);
